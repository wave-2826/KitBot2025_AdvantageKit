...
public class DriveCommands {
   ...
   */
  public static Command driveStraightCommand(
      Drive drive,
      double speedMetersPerSecond,
      Supplier<Rotation2d> fieldAngle,
      ...
      return Commands.sequence(
              Commands.runOnce(
                  () -> {
                    ...
                    if (robotAngle != null)
                      thetaController.setSetpoint(robotAngle.get().getRadians());
                  }),
              Commands.run(
                  () -> {
                    double thetaSpeed =
                    ...
                    drive.runVelocity(
                        ChassisSpeeds.fromFieldRelativeSpeeds(
                            fieldAngleTarget.value.getCos() * speedMetersPerSecond,
                            fieldAngleTarget.value.getSin() * speedMetersPerSecond,
                            thetaSpeed,
                            robotState.getRotation()));
                  },
                  drive))
  ...
  /**
   * Returns a command that drives straight at the specified speed until stopped. Positive numbers
   * are forward.
   */
  public static Command driveStraightCommand(
      Drive drive, double speedMetersPerSecond, Supplier<Rotation2d> robotAngle) {
    return driveStraightCommand(
        drive, speedMetersPerSecond, RobotState.getInstance()::getRotation, robotAngle);
  }

  /**
   * Returns a command that drives straight at the specified speed for the specified duration.
  ...
  /**
   * Returns a command that drives at the specified speed and angle for the specified duration.
   * Positive numbers are forward.
   */
  public static Command driveStraightCommand(
      Drive drive,
      ...
      Supplier<Rotation2d> fieldAngle,
      Supplier<Rotation2d> robotAngle) {
    return driveStraightCommand(drive, speedMetersPerSecond, fieldAngle, robotAngle)
        .withTimeout(timeSeconds);
  }
  ...
  public static Command joystickDriveAtAngle(
    ...
    return Commands.run(
            ...
            drive)
        .beforeStarting( // Reset PID controller when command starts

...
public class DriveToPose extends Command {
  ...
  private static final LoggedTunableNumber setpointMinVelocity =
      new LoggedTunableNumber( //
          ...
          -0.5); // The most negative velocity we will command after velocity correction.
  private static final LoggedTunableNumber minDistanceVelocityCorrection =
      new LoggedTunableNumber( //
          "DriveToPose/MinDistanceVelocityCorrection",
          Units.inchesToMeters(
              0.4)); // When below this distance, we stop correcting the velocity based on the
  ...
  /**
   * A command that drives to the given pose and ends once it gets there. Defaults to a tolerance of
   * 5 inches and 5 degrees.
   */
  ...
  /**
   * A command that drives to the given pose and ends once it gets there if endWhenAtTarget is true.
   * Defaults to a tolerance of 5 inches and 5 degrees.
   */
  public DriveToPose(Drive drive, Supplier<Pose2d> getTarget, boolean endWhenAtTarget) {
    this(
        ...
        getTarget,
        endWhenAtTarget,
        OptionalDouble.of(Units.inchesToMeters(5.)),
        OptionalDouble.of(Units.degreesToRadians(5.)));
  }

  /**
   * A command that drives to the given pose and ends once it gets there. Tolerances are set in
   * meters and radians.
   */
  public DriveToPose(
      Drive drive, Pose2d target, double endLinearTolerance, double endThetaTolerance) {
    this(
        drive,
        () -> target,
        true,
        OptionalDouble.of(endLinearTolerance),
        OptionalDouble.of(endThetaTolerance));
  }

  /**
   * A command that drives to the given pose and ends once it gets there. Tolerances are set in
   * meters and radians. If not present, they will use the default tunable values.
   */
  public DriveToPose(
      Drive drive,
      Supplier<Pose2d> getTarget,
      boolean endWhenAtTarget,
      OptionalDouble endLinearTolerance,
      OptionalDouble endThetaTolerance) {
    this.drive = drive;
  ...
  /**
   * A command that drives to the given pose and never ends. Includes a feedforward for driver
   * controls added to the commanded velocity.
   */
  public DriveToPose(
      Drive drive,
      Supplier<Pose2d> getTarget,
      Supplier<Translation2d> linearDriverFeedforward,
  ...
  /**
   * A command that drives to the given pose and never ends. This constructor is used when we want
   * to use a different pose supplier than the default vision-based odometry estimate.
   */
  ...
  public void execute() {
    ...
    final double linearInterp =
        MathUtil.clamp(linearDriverFeedforward.get().getNorm() * 2.0, 0.0, 1.0);
    final double thetaInterp =
        MathUtil.clamp(Math.abs(thetaDriverFeedforward.getAsDouble()) * 2.0, 0.0, 1.0);

    driveVelocity =
        driveVelocity.interpolate(
            linearDriverFeedforward.get().times(DriveConstants.maxSpeedMetersPerSec), linearInterp);
    thetaVelocity =
        MathUtil.interpolate(
            thetaVelocity,
            thetaDriverFeedforward.getAsDouble() * DriveConstants.maxAngularSpeedRadPerSec,
            thetaInterp);
  ...
  private void updateConstraints() {
    double elevatorT =
        MathUtil.clamp(
            (RobotState.getInstance().getElevatorHeightPercent() - elevatorMinExtension.get())
    ...
    driveProfile =
        new TrapezoidProfile(
            new TrapezoidProfile.Constraints(
                ...
                MathUtil.interpolate(
                    driveMaxAcceleration.get(), driveMaxAccelerationTop.get(), elevatorT)));

    thetaController.setConstraints(
        new TrapezoidProfile.Constraints(
            MathUtil.interpolate(thetaMaxVelocity.get(), thetaMaxVelocityTop.get(), elevatorT),
            MathUtil.interpolate(
                thetaMaxAcceleration.get(), thetaMaxAccelerationTop.get(), elevatorT)));
  }
  ...
  private Translation2d calculateDriveVelocity(Pose2d currentPose, Pose2d targetPose) {
    double linearFeedforwardScalar =
        MathUtil.clamp(
            (absoluteTranslationError - linearFFMinRadius.get())
    ...
    State driveSetpoint =
        driveProfile.calculate(
            ...
            new State(
                direction.norm(),
                -setpointVelocity), // Negative because the profile has zero at the target
            new State(0.0, 0.0));
    ...
    double driveVelocityScalar =
        driveController.calculate(absoluteTranslationError, driveSetpoint.position)
  ...
  private double calculateThetaVelocity(Pose2d currentPose, Pose2d targetPose) {
    double thetaFeedforwardScalar =
        MathUtil.clamp(
            (Units.radiansToDegrees(absoluteThetaError) - thetaFFMinError.get())
  ...
  public boolean atSetpoint(double driveTolerance, Rotation2d thetaTolerance) {
    return running
        && Math.abs(absoluteTranslationError) < driveTolerance
  ...
  public boolean atSetpoint() {
    return atSetpoint(
        endLinearTolerance.orElse(Units.inchesToMeters(driveTolerance.get())),
        Rotation2d.fromRadians(
            endThetaTolerance.orElse(Units.degreesToRadians(thetaTolerance.get()))));
  }
}

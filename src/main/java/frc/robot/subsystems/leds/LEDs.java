...
public class LEDs extends SubsystemBase {
    ...
    public void configureControls() {
        ...
        controller.a().onTrue(Commands.runOnce(() -> {
            enableState(LEDState.Rainbow);
            disableState(LEDState.LightRainbow);
            disableState(LEDState.WaveStripes);
            disableState(LEDState.WaveTwinkle);
            disableState(LEDState.AllianceStripes);
        }));
        controller.b().onTrue(Commands.runOnce(() -> {
            disableState(LEDState.Rainbow);
            enableState(LEDState.LightRainbow);
            disableState(LEDState.WaveStripes);
            disableState(LEDState.WaveTwinkle);
            disableState(LEDState.AllianceStripes);
        }));
        controller.x().onTrue(Commands.runOnce(() -> {
            disableState(LEDState.Rainbow);
            disableState(LEDState.LightRainbow);
            enableState(LEDState.WaveStripes);
            disableState(LEDState.WaveTwinkle);
            disableState(LEDState.AllianceStripes);
        }));
        controller.y().onTrue(Commands.runOnce(() -> {
            ...
            disableState(LEDState.LightRainbow);
            disableState(LEDState.WaveStripes);
            enableState(LEDState.WaveTwinkle);
            disableState(LEDState.AllianceStripes);
        }));
        controller.leftBumper().onTrue(Commands.runOnce(() -> {
            ...
            disableState(LEDState.WaveStripes);
            disableState(LEDState.WaveTwinkle);
            enableState(LEDState.AllianceStripes);
        }));
    ...
    static byte[] twinkleOffsets = null;

    private void waveTwinkle() {
        if(twinkleOffsets == null) {
            twinkleOffsets = new byte[LEDConstants.ledCount];
            var rand = new Random();
            rand.nextBytes(twinkleOffsets);
        }

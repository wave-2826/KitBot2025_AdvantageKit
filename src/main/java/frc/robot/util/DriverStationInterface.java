...
public class DriverStationInterface {
  /**
   * The port used for the driver station interface API. The FMS firewall allows traffic for team
   * use on ports 5800-5810. 5810 is already used by NetworkTables.
   */
  ...
  private LoggedNetworkString reefBranchEntry =
      new LoggedNetworkString("/DriverStationInterface/ReefBranch", "A");
  /** The selected reef level entry in the NetworkTables table. */
  private LoggedNetworkString reefLevelEntry =
      new LoggedNetworkString("/DriverStationInterface/ReefLevel", "L1");
  /**
   * The current robot rotation (in radians) entry in the NetworkTables table. We currently don't
   * just use the pose because deserializing it is a bit difficult.
   */
  private LoggedNetworkNumber robotRotationEntry =
      new LoggedNetworkNumber("/DriverStationInterface/RobotRotation", 0);
  /** The current remaining match time (in seconds) entry in the NetworkTables table. */
  private LoggedNetworkNumber matchTimeEntry =
      new LoggedNetworkNumber("/DriverStationInterface/MatchTime", -1);
  /**
   * The robot state entry in the NetworkTables table. This is used to determine if the robot is in
   * autonomous, teleop, test, or is disabled.
   */
  private LoggedNetworkString robotStateEntry =
      new LoggedNetworkString("/DriverStationInterface/RobotState", "Disabled");

  /**
   * The current robot x position (in meters) entry in the NetworkTables table. We currently don't
   * just use the pose because deserializing it is a bit difficult.
   */
  private LoggedNetworkNumber robotXEntry =
      new LoggedNetworkNumber("/DriverStationInterface/RobotX", 0);
  /**
   * The current robot y position (in meters) entry in the NetworkTables table. We currently don't
   * just use the pose because deserializing it is a bit difficult.
   */
  private LoggedNetworkNumber robotYEntry =
      new LoggedNetworkNumber("/DriverStationInterface/RobotY", 0);
  ...
  private String getAutoData() {
    ...
    try {
      ...
      for (String choice : autoNames) {
        ...
        PathPlannerTrajectory simulatedPath = simulateAuto(pathGroup, DriveConstants.ppConfig);
        ...
        for (double t = 0; t < totalTime; t += sampleInterval) {
          ...
          jsonData
              .append("{\"x\": ")
              .append(Math.round(state.pose.getX() * 1000) / 1000.0)
              .append(", \"y\": ")
              .append(Math.round(state.pose.getY() * 1000) / 1000.0)
              .append(", \"rot\": ")
              .append(Math.round(state.pose.getRotation().getRadians() * 1000) / 1000.0)
              .append(", \"t\": ")
              .append(Math.round(t * 1000) / 1000.0)
              .append("},");
          // @formatter:on
        }
  ...
  static PathPlannerTrajectory simulateAuto(List<PathPlannerPath> paths, RobotConfig robotConfig) {
    ...
    Pose2d startPose =
        new Pose2d(
            firstPath.getAllPathPoints().get(0).position,
    ...
    for (PathPlannerPath p : paths) {
      PathPlannerTrajectory simPath =
          new PathPlannerTrajectory(p, startSpeeds, startPose.getRotation(), robotConfig);

      PathPlannerTrajectoryState lastState =
          allStates.isEmpty() ? null : allStates.get(allStates.size() - 1);
  ...
  public ReefTarget getReefTarget() {
    try {
      return new ReefTarget(
          ReefBranch.valueOf(reefBranchEntry.get()), ReefLevel.valueOf(reefLevelEntry.get()));
